//! UI-side behavior for Profile.json saves (currencies and orbital
//! workshop research). The data model and serialization live in
//! `shared::save`.

use crate::data::ACCOUNT_FLAG_INDEX;
use crate::talent_owner::{FlagOwner, TalentOwner};
use shared::save::TalentEntry;
use shared::Category;

pub use shared::save::{parse_profile_file, serialize_profile_file, ProfileSave};

impl TalentOwner for ProfileSave {
    ...
    fn flag_index(name: &str) -> Option<i64> {
        ACCOUNT_FLAG_INDEX.get(name).copied()
    }
}

impl FlagOwner for ProfileSave {
